...
export const convertKeysToCamelCase = (object: any) => {
  if (isObject(object) && typeof object !== 'undefined' && !(object instanceof Date)) {
    const n: any = {}
  ...
  switch (tableName) {
    case Tables.BANK_DETAILS:
      return (
        ...
        `'bank_account_owner_full_name', ${getFormattedPrefix(0, prefixes)}bank_account_owner_full_name,` +
        `'bank_account_owner_id_number', ${getFormattedPrefix(0, prefixes)}bank_account_owner_id_number,` +
        `'bank_account_number', ${getFormattedPrefix(0, prefixes)}bank_account_number,` +
    ...
    case Tables.ACCOUNTS:
      return (
        ...
        `'authorization_status', ${getFormattedPrefix(0, prefixes)}authorization_status,` +
        ...
        `'apartment_number', ${getFormattedPrefix(0, prefixes)}apartment_number,` +
    ...
    case Tables.USERS:
      return (
        ...
        `'is_active', ${getFormattedPrefix(0, prefixes)}is_active,` +
        `'active_directory_uuid', ${getFormattedPrefix(0, prefixes)}active_directory_uuid,` +
        ...
        `'id_number_country_of_issue', ${getFormattedPrefix(0, prefixes)}id_number_country_of_issue,` +
        `'last_active_at', ${getFormattedPrefix(0, prefixes)}last_active_at,` +
        `'newsletter_subscription', ${getFormattedPrefix(0, prefixes)}newsletter_subscription,` +
    ...
    case Tables.TRANSACTIONS:
      return (
        ...
        `'product_category_other', ${getFormattedPrefix(0, prefixes)}product_category_other,` +
        `'product_subcategory_id', ${getFormattedPrefix(0, prefixes)}product_subcategory_id,` +
        `'product_subcategory_other', ${getFormattedPrefix(0, prefixes)}product_subcategory_other,` +
        `'creator_side', ${getFormattedPrefix(0, prefixes)}creator_side,` +
        `'amount_currency', ${getFormattedPrefix(0, prefixes)}amount_currency,` +
        `'amount', ${getFormattedPrefix(0, prefixes)}amount,` +
        `'commission_id', ${getFormattedPrefix(0, prefixes)}commission_id,` +
        `'commission_payer', ${getFormattedPrefix(0, prefixes)}commission_payer,` +
        `'commission_amount_currency', ${getFormattedPrefix(0, prefixes)}commission_amount_currency,` +
        `'commission_amount', ${getFormattedPrefix(0, prefixes)}commission_amount,` +
        `'end_date', ${getFormattedPrefix(0, prefixes)}end_date,` +
        `'cancel_reason', ${getFormattedPrefix(0, prefixes)}cancel_reason,` +
        `'cancel_reason_other', ${getFormattedPrefix(0, prefixes)}cancel_reason_other,` +
        `'created_at', ${getFormattedPrefix(0, prefixes)}created_at,` +
        `'updated_at', ${getFormattedPrefix(0, prefixes)}updated_at`
      )
    case Tables.PRODUCT_PROPERTIES:
      return (
        ...
        `'line_count_text', ${getFormattedPrefix(0, prefixes)}line_count_text,` +
    ...
    case Tables.COMPANY_DETAILS:
      return (
        `'id', ${getFormattedPrefix(0, prefixes)}id,` +
        `'company_identity_number', ${getFormattedPrefix(0, prefixes)}company_identity_number,` +
        `'incorporation_name', ${getFormattedPrefix(0, prefixes)}incorporation_name,` +
        `'incorporation_date', ${getFormattedPrefix(0, prefixes)}incorporation_date,` +
        `'incorporation_country', ${getFormattedPrefix(0, prefixes)}incorporation_country,` +
        `'funds_source', ${getFormattedPrefix(0, prefixes)}funds_source,` +
        `'funds_source_other', ${getFormattedPrefix(0, prefixes)}funds_source_other,` +
...
export const buildConditionString = (terms: conditionTerm[]) => {
  const sql: string[] = []
  terms.forEach((term) => {
    const value = _.isArray(term.value) ? "('" + term.value.join("','") + "')" : `'${term.value}'`
