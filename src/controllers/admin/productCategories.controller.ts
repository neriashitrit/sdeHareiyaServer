...
import { productCategoryModel, productPropertyModel, productSubcategoryModel } from '../../models/index'
...
export const createProductCategories = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (!_.isNil(files)) {
        Object.keys(files).forEach((key) => {
          if (key.includes('.')) {
            body.subcategories[+key.substring(0, key.indexOf('.'))].icon = files[key]
          }
        })
      }
    } catch (error) {
    ...
    const productCategoryIcon = await fileHelper.uploadFile(Tables.PRODUCT_CATEGORIES, productCategory.id, icon)

    properties.forEach((property: Partial<IProductProperty>) => (property.productCategoryId = productCategory.id))

    const productCategoryProperties = await productPropertyModel.createProductProperties(properties)

    const productSubcategories = await productSubcategoryModel.createProductSubcategories(
    ...
    for (const productSubcategory of productSubcategories) {
      productSubcategory.icon = (
        await fileHelper.uploadFile(Tables.PRODUCT_SUBCATEGORIES, productSubcategory.id, icon)
      ).url
    }
