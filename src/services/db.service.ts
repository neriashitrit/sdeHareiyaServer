...
export default class DbService {
  ...
  getOne = (tableName: string, condition: Record<string, any> = {}) =>
    this.knex.select().from(tableName).where(condition).first()

  getAll = (tableName: string, condition: Record<string, any> = {}) =>
    this.knex.select().from(tableName).where(condition)

  insert = (tableName: string, records: Record<string, string | number | boolean>[]) =>
    this.knex.insert(records, ['*']).into(tableName)

  insertOne = (tableName: string, record: Record<string, any>) => this.insert(tableName, [record])

  update = (tableName: string, updatedRecord: Record<string, any>, condition: Record<string, any> = {}) =>
    this.knex(tableName).update(updatedRecord).where(condition).returning('*')

  updateOneById = async (tableName: string, updatedRecord: Record<string, any>, id: number) =>
