...
export const accountModel = {
  ...
  createAccount: async (newAccount: Record<string, string | number | boolean>): Promise<IAccount> => {
    try {
  ...
  updateAccount: async (
    ...
    try {
      ...
      const account = await db.update(Tables.ACCOUNTS, updatedAccount, parsedCondition)
      return account
    } catch (error) {
  ...
  createCompanyDetails: async (updatedCompanyDetails: Record<string, any>): Promise<ICompanyDetails[]> => {
    try {
      const companyDetails = await db.insert(Tables.COMPANY_DETAILS, updatedCompanyDetails)
      return companyDetails
    } catch (error) {
      console.error('ERROR in Accounts.modal updateCompanyDetails()', error.message)
      throw {
