...
import { convertKeysToCamelCaseArray } from '../utils/helperFunctions';
...
export const accountModel = {
  getAllAccounts: async (startDate?:string,endDate?:string): Promise<IAccount[]> => {
    try {       
      ...
      db.knex
      .select('accounts.*', db.knex.raw('coalesce(summary.active_transaction_count, 0) as active_transaction_count'), db.knex.raw('json_agg(users) as users'))
      .from('accounts')
      ...
      .leftJoin(
        db.knex
          ...
          .from('user_accounts')
          .leftJoin('transaction_sides', 'user_accounts.account_id', 'transaction_sides.user_account_id')
          .leftJoin('transactions', 'transactions.id', 'transaction_sides.transaction_id')
      ...
      .modify((queryBuilder) => {
        if (startDate && endDate) {
          ...
          queryBuilder.whereBetween('users.last_active_at', [`${startDate} 00:00:00`, `${endDate} 23:59:59`]);
        } else if (startDate) {
          // If only the start date is specified, filter by that date and later
          queryBuilder.where('users.last_active_at', '>=', `${startDate} 00:00:00`);
        } else if (endDate) {
          // If only the end date is specified, filter by that date and earlier
          queryBuilder.where('users.last_active_at', '<=', `${endDate} 23:59:59`);
        }
      })
      .groupBy('accounts.id','summary.active_transaction_count')    
      return convertKeysToCamelCaseArray(accounts);
    } catch (error) {
      console.error(
