...
export const commissionModel = {
  getCommissions: async (condition: Record<string, any> | string): Promise<ICommission[]> => {
    try {
  ...
  createCommission: async (commission: Record<string, number | CommissionType | boolean>): Promise<ICommission> => {
    try {
